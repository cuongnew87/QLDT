    ...
    public partial class Compose : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Request.Params["id"] != null && int.TryParse(Request.Params["id"], out id))
            {
                ...
                {
                    db.conn.Open();
                    string query = "select email from Comments where id = '" + id + "'";
                    SqlCommand cmd = new SqlCommand(query, db.conn);
                    string To = cmd.ExecuteScalar().ToString();
                    db.conn.Close();

                    txtTo.Text = To;
                    txtSubject.Text = "Reply Comment";
                }
            }
        }
        ...
        protected void btnSend_Click(object sender, EventArgs e)
        {
            ...
            mail.To.Add(email);
            mail.From = new MailAddress(Session["email"].ToString(), "Known", System.Text.Encoding.UTF8);
